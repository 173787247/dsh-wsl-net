...
/**
 * Clash often injects RFC1918 / wildcard NO_PROXY. Node may then skip the proxy
 * for traffic that still needs it — use kit restart-dsh-web.sh (loopback-only NO_PROXY).
 */
export function riskyNoProxy(noProxy) {
  const s = String(noProxy || "").trim();
  if (!s) return false;
  const onlyLoopback = /^127\.0\.0\.1\s*,\s*localhost$/i.test(s) || /^localhost\s*,\s*127\.0\.0\.1$/i.test(s);
  if (onlyLoopback) return false;
  return (
    /\b10\.\*/.test(s) ||
    /\b10\.\d/.test(s) ||
    /\b172\.(1[6-9]|2\d|3[01])\./.test(s) ||
    /\b192\.168\./.test(s) ||
    /\*/.test(s)
  );
}
...
export function buildAdvice(env, probes, target, injectChildProxy, proxyListen = null, dshWeb = null) {
  ...
  const noProxyVal = env.NO_PROXY || hostEnv?.NO_PROXY || "";
  if (proxyOn(env) && riskyNoProxy(noProxyVal)) {
    lines.push(
      `NO_PROXY looks like Clash RFC1918/wildcards (${redactSecretUrl(noProxyVal) || "set"}) — Node may bypass the proxy incorrectly. Prefer kit restart-dsh-web.sh which sets NO_PROXY=127.0.0.1,localhost only.`,
    );
  }
